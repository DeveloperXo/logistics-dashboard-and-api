...
export const getCustomers = (axiosPrivate, query) => async (dispatch) => {
    try {
        ...
        if (query) {
            response  = await axiosPrivate.get(`/customer/customer/${query?.page}/${query?.limit}`, {
                signal: controller.signal
            });
        } else {
            response  = await axiosPrivate.get(`/customer/customer/1/10`, {
                signal: controller.signal
            });
        }
...
export const fetchCustomer = async (id, axiosPrivate) => {
    try {
        const response = await axiosPrivate.get(`customer/customer/${id}`, {
            headers: { 'Content-Type': 'application/json' }
        });
        return response;
    } catch (err) {
